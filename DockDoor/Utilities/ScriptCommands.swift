...
enum DockDoorCommands {
    ...
    enum CommandError: Error, LocalizedError {
        ...
        var errorDescription: String? {
            ...
            case .coordinatorNotAvailable:
                return "DockDoor coordinator not available"
            case .invalidPosition(let pos):
                return "Invalid position: \(pos). Use: left, right, top, bottom, top-left, top-right, bottom-left, bottom-right"
...
class WindowActionCommand: NSScriptCommand {
    override func performDefaultImplementation() -> Any? {
        ...
        // Determine action from command description
        guard let commandDesc = commandDescription.commandName else {
        ...
        case "center window":
            action = .center
        default:
            return "error: Unknown action: \(commandDesc)"
        }
...
class PositionWindowCommand: NSScriptCommand {
    override func performDefaultImplementation() -> Any? {
        ...
        guard let positionString = evaluatedArguments?["position"] as? String,
              let position = DockDoorCommands.WindowPosition(rawValue: positionString) else {
